...
fn main() -> PyResult<()> {
    ...
    file_timestamps.insert("F:\\WindowsDesktop\\Users\\Leamy\\Desktop\\ChatPerfect\\src\\users\\patrick_leamy\\database\\changed-flag.txt".to_string(), SystemTime::now());
    ...
    // The files monitored are the changed-flag.txt file and the llm-response.txt file
