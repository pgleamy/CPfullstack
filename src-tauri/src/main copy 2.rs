...
fn main() -> PyResult<()> {
    ...
    // File polling logic
    let polling_thread = thread::spawn(|| {
        let file_path = "F:\\WindowsDesktop\\Users\\Leamy\\Desktop\\ChatPerfect\\src\\backend\\totalDatabaseEntries.txt";
        let mut last_modified: SystemTime = metadata(file_path).unwrap().modified().unwrap();
        ...
        loop {
            ...
            match metadata(file_path) {
                ...
                Err(e) => println!("Error polling file: {}", e),
            }
        }
    });
    ...
    polling_thread.join().unwrap(); // Add this line
